...
-   Middle line: median (very close to mean b/c looks close to it in the plot)

-   Box: the middle 50% of the data(25%-75%) InterQuartileRange(IQR)

-   Whiskers: they extend out 1.5 times the IQR

-   Dots: basically the outliers
...
ggplot(diamonds, aes(x = carat, y = price)) +
  geom_point(alpha = 0.05) # try making the points transparent to 0.05. (there are striations, due to exact half carats).
```
...
Is there a relationship between `color` and `price`? Yes, negative correlation, as quality increases,
...
-   Where do we see a lot of variability? with the clarity vs price. the p value is very low, thus it is hard to see a clear relationship between the 2 variables.
